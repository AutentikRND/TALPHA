...
//#include <Adafruit_SensorS.h>//Library Sensor Adafruit
...
#define TFT_RST   D3
#define TFT_DC    D4
//#define TFT_MOSI    D7     // TFT MOSI  pin is connected to NodeMCU pin D7
//#define TFT_SCK    D5     // TFT SCK  pin is connected to NodeMCU pin D5
#define SD_CS 10
#define bat A0
...
//AnalogSmooth as = AnalogSmooth();
            ...
            //ID Device
            String idDevice = "53d75ce5-2b60-4c02-8615-acdd643706fc";
...
void setup() {
  ...
  tft.setTextColor(ILI9341_WHITE);
  tft.setTextSize(1);
  tft.println("Firmware Version 1.0.5");
  tft.println(" ");
  tft.setTextColor(ILI9341_GREEN);
  tft.setTextSize(1);
  ...
  bool status;
  sensor.begin();
  Serial.println("TEST SENSOR DULU BOY");
...
void loop() {
  ...
  if (currentMillis - previousMillis >= period) { // check if 1000ms passed
    ...
    //te = a + b * tempC + c * (pow(tempC, 2)); //regresi temp
...
void Create_Data() {
  ...
  Serial.println("SUCCES - SD card initialization done!");
}
...
void testText(float caltempC, float calhumidity) {
  //Uncommand code dibawah untuk versi talpha terbaru
  /*tft.setCursor(10, 300);
    tft.setTextColor(ILI9341_CYAN);
    tft.setTextSize(0.5);
    tft.println(name);
    //suhu
    tft.drawBitmap(185, 97 , suhu, 50, 38, ILI9341_CYAN);
    tft.setCursor(42, 110);
    tft.setTextSize(3);
    tft.setTextColor(ILI9341_WHITE, ILI9341_BLACK);
    tft.print(String(caltempC, 1));
    //KELEMBABAN
    tft.drawBitmap(195, 173, humi, 20, 20, ILI9341_CYAN);
    tft.setCursor(42, 180);
    tft.setTextSize(3);
    tft.setTextColor(ILI9341_WHITE, ILI9341_BLACK);
    tft.print(String(calhumidity, 1));*/
  //Uncommand code dibawah untuk versi talpha terbaru
  tft.setCursor(10, 300);
  tft.setTextColor(ILI9341_WHITE, ILI9341_BLACK);
  tft.setTextSize(1);
